...
$table=$db->dbname.'.petty_cash';
$table_items=$db->dbname.'.petty_cash_items';
...
if($action=='add'){
    if($documentID==''){
        $documentID=$db->genarate_docuemntID('petty_cash','documentID','C-ym',4);
    }
$params=array(
'comCode'=>$db->comCode,
        ...
        if($chargeitems){
         foreach($chargeitems as $items){
            $params_items=array(
                    ...
                    $db->insertData('petty_cash_items',$params_items); 
             $sum_amount+=$amount[$i];
             $i++;
         }
         $success_status=1;
