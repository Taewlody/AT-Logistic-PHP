...
<div class="row wrapper border-bottom white-bg page-heading">
  <div class="col-lg-10">
    ...
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item"> <a>Port</a> </li>
    </ol>
  </div>
    ...
    <div class="col-lg-12">
      <div class="ibox ">
        <div class="ibox-title">
          <h5>port  List</h5>
          <div class="ibox-tools"> <a href="port_form?action=add" class="btn btn-primary btn-xs"><i class="fa fa-plus "> </i> Create new </a> </div>
        </div>
...
INNER JOIN $db->dbname.common_country AS ct ON c.comCode = ct.comCode AND c.countryCode = ct.countryCode
INNER JOIN $db->dbname.ref_active AS a ON c.comCode = a.comCode AND c.isActive = a.`code`
INNER JOIN $db->dbname.user AS u ON c.comCode = u.comCode AND c.createID = u.usercode order by c.portCode  ";
$result=$db->query($sql);              
$i=1;
$isActiveStype="";
...
?>
                <tr class="gradeX">
                  <td><?php echo $i++; ?></td>
                  <td><?php echo $r['portCode'];?></td>
                  <td><?php echo $r['portNameTH']; ?></td>
                  ...
                  <td class="center">
                    <div class="btn-group">
                      <button class="btn-white btn btn-xs" onClick="location.href='port_form?action=view&portCode=<?php echo $r['portCode'];?>' ">View</button>
                      <button class="btn-white btn btn-xs" onClick="location.href='port_form?action=edit&portCode=<?php echo $r['portCode'];?>' ">Edit</button>
                      <button class="btn-white btn btn-xs" onClick="return confirmDel('<?php echo $r['portCode']; ?>','port_action.php');">Delete</button>
                    </div>
                  </td>
                </tr>
<?php  
