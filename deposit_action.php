...
$table=$db->dbname.'.deposit';
$table_items=$db->dbname.'.deposit_items';
...
if($action=='add'){
    if($documentID==''){
        $documentID=$db->genarate_docuemntID('deposit','documentID','PV-ym',4);
    }
$params=array(
'comCode'=>$db->comCode,
        ...
        if($chargeitems){
         foreach($chargeitems as $items){
            ...
            $params_items=array(
                    ...
                    $db->insertData('deposit_items',$params_items); 
             $sum_amount+=$amount[$i];
             $i++;
         }
         $success_status=1;
...
foreach($imgKey as $r){
    $params=array(
        'comCode'=>$db->comCode,
        'documentID'=>$documentID);
        $db->updateData('deposit_attach',$params,"comCode='$db->comCode' AND fileName='$r'");
