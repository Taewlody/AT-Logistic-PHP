...
$uploaddir = 'supplier_path/'.$supCode.'/';

      if (file_exists("supplier_path/$supCode")) {
      }else{
        mkdir("supplier_path/$supCode", 0700);
      }
...
if(move_uploaded_file($_FILES["attach_file"]["tmp_name"], $uploaddir.$fileName)){
        $db->insertData('shiping_payment_voucher_attach', $params);
        $array = array( 
        "fileName"=>$fileName,
        "result"=>'success');
