    ...
    <div class="col-lg-12">
      <div class="ibox ">
        <div class="ibox-title">
          <h5>Charges List</h5>
          <div class="ibox-tools"> <a href="charges_form?action=add" class="btn btn-primary btn-xs"><i class="fa fa-plus "> </i> Create new </a> </div>
        </div>
...
INNER JOIN $db->dbname.common_chargestype AS v ON c.comCode = v.comCode AND c.typeCode = v.typeCode
INNER JOIN $db->dbname.ref_active AS a ON c.comCode = a.comCode AND c.isActive = a.`code`
INNER JOIN $db->dbname.user AS u ON c.comCode = u.comCode AND c.createID = u.usercode order by c.chargeCode  ";
$result=$db->query($sql);
$i=1;
$isActiveStype="";
...
?>
                <tr class="gradeX">
                  <td><?php echo $i++; ?></td>
                  <td><?php echo $r['chargeCode'];?></td>
                  <td><?php echo $r['chargeName']; ?></td>
                  ...
                  <td class="center">
                    <div class="btn-group">
                      <button class="btn-white btn btn-xs" onClick="location.href='charges_form?action=view&chargeCode=<?php echo $r['chargeCode'];?>' ">View</button>
                      <button class="btn-white btn btn-xs" onClick="location.href='charges_form?action=edit&chargeCode=<?php echo $r['chargeCode'];?>' ">Edit</button>
                      <button class="btn-white btn btn-xs" onClick="return confirmDel('<?php echo $r['chargeCode']; ?>','charges_action.php');">Delete</button>
                    </div>
                </tr>
<?php  
}
