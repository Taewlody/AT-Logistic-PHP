...
$table=$db->dbname.'.receipt_voucher';
$table_items=$db->dbname.'.receipt_voucher_items';
...
if($action=='add'){
    if($documentID==''){
        $documentID=$db->genarate_docuemntID('receipt_voucher','documentID','RV-ym',4);
    }
$params=array(
'comCode'=>$db->comCode,
        ...
        if($chargeitems){
         foreach($chargeitems as $items){
            ...
            $params_items=array(
                    ...
                    $db->insertData('receipt_voucher_items',$params_items); 
             $sum_amount+=$amount[$i];
             $i++;
         }
         $success_status=1;
