    ...
    <div class="col-lg-12">
      <div class="ibox ">
        <div class="ibox-title">
          ...
          <div class="ibox-tools"> <a href="user_form?action=add" class="btn btn-primary btn-xs"><i class="fa fa-plus "> </i> Create new </a> </div>
        </div>
...
a.acttiveName,
`at`.userTypeName,
us.`username` AS createName
FROM $db->dbname.user AS u
INNER JOIN $db->dbname.ref_active AS a ON u.comCode = a.comCode AND u.isActive = a.`code`
INNER JOIN $db->dbname.user AS us ON u.comCode = us.comCode AND u.createID = us.usercode 
INNER JOIN user_type as at on `at`.comCode=u.comCode AND `at`.userTypecode=u.userTypecode
  
";
                  
if($_SESSION['userTypecode']!='1'){  
 $sql .=" WHERE  u.usercode='$_SESSION[userID]'"; 
}              
...
?>
                <tr class="gradeX">
                  ...
                  <td class="center"><?php echo $r['createName'];?></td>
                  <td class="center">
                    <div class="btn-group">
                      <button class="btn-white btn btn-xs" onClick="location.href='user_form?action=view&usercode=<?php echo $r['usercode'];?>' ">View</button>
                      <button class="btn-white btn btn-xs" onClick="location.href='user_form?action=edit&usercode=<?php echo $r['usercode'];?>' ">Edit</button>
                      <button class="btn-white btn btn-xs" onClick="return confirmDel('<?php echo $r['usercode']; ?>','user_action.php');">Delete</button>
                    </div>
                </tr>
<?php  
}
