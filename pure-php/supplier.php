    ...
    <div class="col-lg-12">
      <div class="ibox ">
        <div class="ibox-title">
          <h5>Supplier List</h5>
          <div class="ibox-tools"> <a href="supplier_form?action=add" class="btn btn-primary btn-xs"><i class="fa fa-plus "> </i> Create new </a> </div>
        </div>
...
a.acttiveName,
u.username
FROM $db->dbname.common_supplier AS c
INNER JOIN $db->dbname.ref_active AS a ON c.comCode = a.comCode AND c.isActive = a.`code`
INNER JOIN $db->dbname.user AS u ON c.comCode = u.comCode AND c.createID = u.usercode order by c.supCode  ";
$result=$db->query($sql);
$i=1;
$isActiveStype="";
...
?>
                <tr class="gradeX">
                  ...
                  <td class="center">
                    <div class="btn-group">
                      <button class="btn-white btn btn-xs" onClick="location.href='supplier_form?action=view&supCode=<?php echo $r['supCode'];?>' ">View</button>
                      <button class="btn-white btn btn-xs" onClick="location.href='supplier_form?action=edit&supCode=<?php echo $r['supCode'];?>' ">Edit</button>
                      <button class="btn-white btn btn-xs" onClick="return confirmDel('<?php echo $r['supCode']; ?>','supplier_action.php');">Delete</button>
                    </div>
                </tr>
<?php  
}
