...
INNER JOIN $db->dbname.joborder AS j ON p.comCode = j.comCode AND p.refJobNo = j.documentID
INNER JOIN $db->dbname.common_customer AS cs ON j.comCode = cs.comCode AND j.cusCode = cs.cusCode
...
class MYPDF extends TCPDF {
	...
	public function Header() {
       ...
       // $this->SetTextColor(0, 139, 139);
		$this->SetFont('thsarabun', 'B', 18);
		$this->Cell(0, 15,'เงินสำรองจ่ายล่วงหน้า', 0, false, 'C', 0, '', 0, false, 'M', 'M');  
		$this->Ln(5);
		$this->Cell(0, 15,'ADVANCE PAYMENT', 0, false, 'C', 0, '', 0, false, 'M', 'M');  
...
t.chartDetail,
t.amount
FROM  $db->dbname.advance_payment_items AS t
