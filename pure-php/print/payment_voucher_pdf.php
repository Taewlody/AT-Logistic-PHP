...
class MYPDF extends TCPDF {
	...
	public function Header() {
       ...
       $this->Ln(3);
		...
		$this->Rect(75, 28, 60, 12, 'DF', $style, array(255, 99, 71));
        
		$this->Ln(6);
       // $this->SetTextColor(0, 139, 139);
		$this->SetFont('thsarabun', 'B', 18);
		$this->Cell(0, 15,'ใบสำคัญจ่าย', 0, false, 'C', 0, '', 0, false, 'M', 'M');  
		$this->Ln(5);
		$this->Cell(0, 15,'PAYMENT VOUCHER', 0, false, 'C', 0, '', 0, false, 'M', 'M');  
        ...
        $this->SetX(50);
        $this->SetFont('thsarabun', '', 15);
        $this->Cell(0, 15,$_SESSION['refJobNo'].'/'.$_SESSION['invNo'], 0, false, 'L', 0, '', 0, false, 'M', 'M');
        ...
        $this->SetX(125);
        $this->SetFont('thsarabun', 'B', 15);
        $this->Cell(0, 15,'ชื่อบัญชี', 0, false, 'L', 0, '', 0, false, 'M', 'M');
        $this->SetX(155);
        $this->SetFont('thsarabun', '', 15);
        $this->Cell(0, 15,$_SESSION['accountNicname'], 0, false, 'L', 0, '', 0, false, 'M', 'M');  
...
p.sumTax7,
p.grandTotal
FROM  $db->dbname.payment_voucher_items AS t
INNER JOIN $db->dbname.payment_voucher AS p ON t.comCode = p.comCode AND t.documentID = p.documentID
...
if ($r['tax'] == 1) {
    $sumary_tax1    += $r['amount'];
}
if ($r['tax'] == 3) {
    $sumary_tax3    += $r['amount'];
}
...
$html.='
<tr>
	<td align="center" colspan="2" rowspan="1" width="458px"></td>
	<td align="left" width="100px">Total </td>
    <td align="right">'.number($sumTotal,2).'</td>		
</tr> 
<tr>
	<td align="center" colspan="1" rowspan="2" width="358px"></td>
	<td align="left" width="100px">'.number($sumary_tax1,2).'</td>
	<td align="left">Tax 1% </td>
    <td align="right">'.number($sumTax1,2).'</td>		
</tr> 
<tr>
    <td align="left" width="100px">'.number($sumary_tax3,2).'</td>
    <td align="left">Tax 3% </td>
    <td align="right">'.number($sumTax3,2).'</td>		
</tr> 
<tr>
    <td align="center" colspan="2" rowspan="2" width="458px"></td>
	<td align="left">Vat Total</td>
