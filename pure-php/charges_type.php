    ...
    <div class="col-lg-12">
      <div class="ibox ">
        <div class="ibox-title">
          ...
          <div class="ibox-tools"> <a href="charges_type_form?action=add" class="btn btn-primary btn-xs"><i class="fa fa-plus "> </i> Create new </a> </div>
        </div>
...
$sql="SELECT
	...
	a.acttiveName,
	u.username ,
  v.typeName as vatTypeName
FROM $db->dbname.common_chargestype AS c
INNER JOIN $db->dbname.common_vattype AS v ON c.comCode = v.comCode AND c.vatType = v.typeCode
INNER JOIN $db->dbname.ref_active AS a ON c.comCode = a.comCode AND c.isActive = a.`code`
INNER JOIN $db->dbname.user AS u ON c.comCode = u.comCode AND c.createID = u.usercode order by c.typeCode  ";
$result=$db->query($sql);
$i=1;
$isActiveStype="";
...
?>
                <tr class="gradeX">
                  <td><?php echo $i++; ?></td>
                  <td><?php echo $r['typeCode'];?></td>
                  <td><?php echo $r['typeName']; ?></td>
                  <td><?php echo $r['vatTypeName']; ?></td>
                  ...
                  <td class="center">
                    <div class="btn-group">
                      <button class="btn-white btn btn-xs" onClick="location.href='charges_type_form?action=view&typeCode=<?php echo $r['typeCode'];?>' ">View</button>
                      <button class="btn-white btn btn-xs" onClick="location.href='charges_type_form?action=edit&typeCode=<?php echo $r['typeCode'];?>' ">Edit</button>
                      <button class="btn-white btn btn-xs" onClick="return confirmDel('<?php echo $r['typeCode']; ?>','charges_type_action.php');">Delete</button>
                    </div>
                </tr>
<?php  
}
