...
$dateEnd=date('d/m/Y');
*/
$dateStart=isset($_POST['dateStart'])?$_POST['dateStart']: date("d/m/Y", strtotime("-2 months"));
...
FROM
invoice AS iv
INNER JOIN tax_invoice_items AS tx ON iv.comCode = tx.comCode AND iv.documentID = tx.invNo
WHERE iv.ref_jobNo=j.documentID
LIMIT 1
) as iv
...
?>
                <tr>
                  ...
                  <td class="center"> <span class="label label-<?php echo $isActiveStype;?>"><?php  if($r['iv']!=''){ echo 'tax complete'; }else{   echo $r['status_name'];} ?></span></td>
                  <td class="center">
                    <div class="btn-group">
					...
					<?php 	
	if($r['documentstatus']=='P' || $_SESSION['userTypecode']=='1'){?>		
						
                      <button  class="btn-white btn btn-xs" onClick="location.href='job_form?action=edit&documentID=<?php echo $r['documentID'];?>' " <?php if($r['documentstatus']=='A'){ /*echo 'disabled';*/}?> >Edit</button>
						<?php } ?>
                        <button class="btn-white btn btn-xs" onClick="location.href='job_form?action=copy&documentID=<?php echo $r['documentID'];?>' ">Copy</button>
	...
	if($r['documentstatus']=='P' || $_SESSION['userTypecode']=='1'){?>	
                      ...
                      <button class="btn-white btn btn-xs" onClick="return confirmDel('<?php echo $r['documentID']; ?>','job_action.php');">Delete</button>
					<?php } ?>
